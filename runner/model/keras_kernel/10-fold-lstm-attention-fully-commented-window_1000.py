...
from keras import \
    backend as K  # The backend give us access to tensorflow operations and allow us to create the Attention class
from keras.callbacks import *  # This object helps the model to train in a smarter way, avoiding overfitting
from keras.layers import *  # Keras is the most friendly Neural Network library, this Kernel use a lot of layers classes
from keras.models import Model
from sklearn.model_selection import StratifiedKFold  # Used to use Kfold to train our model
...
df_train = pd.read_csv(VbsDataSetFactory.TRAIN_METADATA_PATH)
...
def transform_ts(ts, n_dim=80, min_max=(-1, 1)):
    ...
    for i in range(0, sample_size, bucket_size):
        ...
        new_ts.append(
            np.concatenate([np.asarray([mean, std, std_top, std_bot, max_range]), percentil_calc, relative_percentile]))
...
def prep_data(start, end):
    ...
    praq_train = pq.read_pandas(VbsDataSetFactory.TRAIN_PARQUET_PATH,
...
# this piece of code is a function that evaluates all the possible thresholds from 0 to 1 by 0.01ff
def threshold_search(y_true, y_proba):
    best_threshold = 0
    best_score = 0
    for threshold in tqdm([i * 0.01 for i in range(100)]):
        score = K.eval(matthews_correlation(K.variable(y_true.astype(np.float64)),
                                            K.variable((y_proba > threshold).astype(np.float64))))
        if score > best_score:
...
for start, end in start_end:
    subset_test = pq.read_pandas(VbsDataSetFactory.TEST_PARQUET_PATH,
                                 columns=[str(i) for i in range(start, end)]).to_pandas()
