...
from feature.base import SummaryWindowFeature, PhaseStandardScaledFeature
...
def run(window_size, step_size):
    ...
    save_root = Path(
        "/mnt/share/vbs-power-line-fault-detection/features/summary/"
        "/phase_stdscale/window_{}_step_{}".format(
            window_size, step_size))

    summary_feature = SummaryWindowFeature(window_size=window_size, step_size=step_size,
                                           axis=1)
    save_root.mkdir(exist_ok=True, parents=True)
    print("writing train  ....")
    train_src_path = feature_root.joinpath("train/feature.pickle")
    train_save_root = save_root.joinpath("train")
    src_feature = PhaseStandardScaledFeature()

    train_save_root.mkdir(exist_ok=True, parents=True)
    src_feature.load(train_src_path)
    summary_feature.transform(src_feature, train_save_root, n_jobs=-1)
    ...
    summary_feature.transform(src_feature, test_save_root, n_jobs=-1)
