...
from feature.base import FftFeature, WindowFeature


def run(fft_length, fft_stride, n_components):
    src_feature = FftFeature(fft_length=fft_length, fft_stride=fft_stride)
    feature_root = Path(
    ...
    save_root = Path(
        "/mnt/share/vbs-power-line-fault-detection/features/fft/"
        "/pca/fft_length_{}_stride_{}".format(
            fft_length, fft_stride))

    pca_feature = WindowFeature(joblib.load(save_root.joinpath("train/transformer.pickle")),
                                window_size=fft_length, step_size=fft_stride, accept_window=True,
                                output_dim=n_components)
    save_root.mkdir(exist_ok=True, parents=True)
    # print("writing train pca  ....")
    ...
    # pca_feature.fit_transform(src_feature, train_save_root, n_jobs=-1)

    print("writing test pca data....")
    ...
    pca_feature.transform(src_feature, test_save_root, n_jobs=-1)
